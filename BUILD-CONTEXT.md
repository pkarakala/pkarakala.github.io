...
The owner-supplied `DESIGN-BRIEF.md` and `HOMEPAGE-COPY.md` from the planning folder governed the initial content; the later request to change color/font/composition superseded the original visual token suggestions. The owner subsequently approved a more specific hero statement and shorter, hardware-first About copy in September 2026. The current portrait is preserved at `/profile.jpg`; responsive WebP copies keep its complete natural rectangle with no filter.
...
- KernelRelay's public repository and documentation support an evaluation-and-feedback description, not a model-training claim: it uses fixed mock source proposals, validates candidates against eager PyTorch, records rewards and trajectories, and reports measured T4 timings. The portfolio links one same-process T4 confirmation with its variability caveat and explicitly labels the fixed mock.
